...
bmap(struct inode *ip, uint bn)
{
    ...
    if (bn < NDOUBLE)
    {
        ...
        uint first = bn / NINDIRECT;  // index in double-indirect
        uint second = bn % NINDIRECT; // index in single-indirect

        // Allocate single-indirect block if missing
        if ((addr = a[first]) == 0)
        {
        ...
        // Allocate data block if missing
        if ((addr = a2[second]) == 0)
        {
            a2[second] = addr = balloc(ip->dev);
            log_write(bp2);
        }
